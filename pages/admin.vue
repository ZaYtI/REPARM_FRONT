...
const totalPagesProduct = computed(() =>
  Math.ceil(selectCatStore.getAllProducts.length / itemsPerPage.value)
);

const totalPagesOrder = computed(() => {
  const filteredOrders = authStore.getAllOrder.filter(
    (order) => !(order.payment && order.received)
  );
...
const currentProducts = computed(() => {
  const sortedProducts = selectCatStore.getAllProducts.toSorted(
    (a, b) => a.id - b.id
  );
        ...
        <div :class="{ 'd-none': currentProducts == undefined }">
          <div class="d-flex justify-content-between px-3 pb-3">
            ...
            <div class="d-flex">
              <div class="d-flex align-items-center paginate">
                <button
                  @click="currentPageIndexProduct--"
                  class="btn border-0 btn-paginate"
                  :disabled="!previousProductIsVisible"
                >
                  <<
                </button>
                <p class="p-0 m-0">
                  {{ currentPageIndexProduct }}...{{ totalPagesProduct }}
                </p>
                <button
                  @click="currentPageIndexProduct++"
                  class="btn border-0 btn-paginate"
          ...
          <div class="table-responsive rounded-3">
            <table
              ...
              aria-describedby="table of product"
            >
              <thead class="head">
                <tr>
                  ...
                  <td>
                    <div class="d-flex justify-content-around">
                      <button
                        ...
                        data-bs-toggle="modal"
                        data-bs-target="#exampleModal"
                        @click="authStore.setProductToUpdate(product)"
                      >
                        Modifier
                      </button>
                      <button
        ...
        <div>
          <div class="d-flex justify-content-between px-3 pb-3 mt-4">
            ...
            <div class="d-flex align-items-center paginate">
              ...
              <p class="p-0 m-0">
                {{ currentPageIndexOrder }}...{{ totalPagesOrder }}
              </p>
              <button
                @click="currentPageIndexOrder++"
                class="btn border-0 btn-paginate"
          ...
          <div class="table-responsive rounded-3">
            <table
              class="table table-striped"
              aria-describedby="table of order"
            >
              <thead class="head">
                <tr>
                  <th class="text-center">Id</th>
              ...
              <tbody>
                <tr v-for="order in currentOrders" :key="order.id">
                  ...
                  <td class="text-center span-order">
                    <span class="success-order">Valider</span>
                  </td>
                  <td class="text-center span-order" v-if="order.state == 'WAITING'">
                    <span class="wait-order">En attente</span>
                  </td>
                  <td class="text-center span-order" v-if="order.state == 'FAILED'">
                    <span class="fail-order">Abandonner</span>
                  </td>
                  ...
                  <td>
                    <div class="d-flex justify-content-around">
                      <button
                        ...
                        type="button"
                        data-bs-toggle="modal"
                        data-bs-target="#orderModalTop"
                        @click="authStore.setOrderToUpdateSelected(order)"
                        :disabled="order.received && order.payment"
                      >
                        Modifier
                      </button>
                      <button
