...
async function getProductById() {
  if (store.getlistOfSelectedProducts == null || store.getlistOfSelectedProducts == undefined || store.getlistOfSelectedProducts.length == 0) {
    const response = await fetch('https://reparm-front.onrender.com/product/getById/' + router.params.id, {
      method: 'GET',
      headers: {
...
async function addProductToBasket() {
  const response = await fetch('https://reparm-front.onrender.com/panier-item/' + parseInt(router.params.id), {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
